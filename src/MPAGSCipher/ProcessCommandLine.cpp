                        ...
                        ProgramSettings& setting)
{
    ...
    for (std::size_t i{1}; i < nCmdLineArgs; ++i) {
        if (cmdLineArgs[i] == "-h" || cmdLineArgs[i] == "--help") {
            ...
            setting.helpRequested = true;
            break;
        } else if (cmdLineArgs[i] == "--version") {
            // Set the indicator and terminate the loop
            setting.versionRequested = true;
            break;
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                setting.inputFile = cmdLineArgs[i + 1];
                ++i;
            }
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                // Got filename, so assign value and advance past it
                setting.outputFile = cmdLineArgs[i + 1];
                ++i;
            }
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                setting.cipherKey = cmdLineArgs[i + 1];
                ++i;
            }
        } else if (cmdLineArgs[i] == "--encrypt") {
            setting.mode = CipherMode::encrypt;
        } else if (cmdLineArgs[i] == "--decrypt") {
            setting.mode = CipherMode::decrypt;
        } else {
